 ...
 *.gz|*.xpm|*.afm|*.pfb|*.ttf|*.au|*.wav|*.ogg|*.jpg|fonts.*|*.pcf|*.png|*.so|*.so.conf|*.svg|yaf-splash)
 ...
 [ -e "$ONE_DIR_IN_SYSTEM/$ONE_FILE" ] && {
     ...
     PERM_R1=`stat -c %a "$ONE_DIR_IN_SYSTEM/$ONE_FILE"`
     PERM_R2=`stat -c %a ./"$ONE_FILE"`

     OWN_GRP1=`stat -c %U":"%G "$ONE_DIR_IN_SYSTEM/$ONE_FILE"`
     OWN_GRP2=`stat -c %U":"%G ./"$ONE_FILE"`
     ...
     if test "$PERM_R1" != "$PERM_R2"; then
     echo "PERMISSIONS of $ONE_FILE differ: SYS='$PERM_R1' GIT='$PERM_R2'"

     fi

     if test "$OWN_GRP1" != "$OWN_GRP2"; then
     echo "OWNER_GROUP of $ONE_FILE differ: SYS='$OWN_GRP1' GIT='$OWN_GRP2'"

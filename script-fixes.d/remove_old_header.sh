...
do
   ...
   *.txt|*.TXT|*rc|*.xpm|*.svg|*.png|*.jpg|*.wav|*.au|*.bac|*.dat|*.pupdev|*.gs|*Help)
   ...
   gPATTERN1='__old_header__(){  #BEGIN'
   grep -F $QUIET "$gPATTERN1" "$file" || { echo "$file has no '$gPATTERN1'"; continue; }

   [ "$VERBOSE" ] && echo "$file has '$gPATTERN'"

   gPATTERN2='###__old_header__(){  #END'
   grep -F $QUIET "$gPATTERN2" "$file" || { echo "$file has no '$gPATTERN2'"; continue; }

   sed -i".bak" "/$gPATTERN1/,/$gPATTERN2/d" "$file"
   [ $? = 0 ] && rm "${file}.bak" || {

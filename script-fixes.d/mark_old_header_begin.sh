...
do
   ...
   *.txt|*.TXT|*rc|*.xpm|*.svg|*.png|*.jpg|*.wav|*.au|*.bac|*.dat|*.pupdev|*.gs|*Help)
   ...
   gPATTERN='__old_header__(){'
   grep -F $QUIET "$gPATTERN" "$file" || continue
   grep -F "$gPATTERN" "$file" | grep $QUIET 'BEGIN' && continue

   [ "$VERBOSE" ] && echo "$file needs adjustment"
   if [ ! "$DRY" ]; then

   sed -i".bak" "s,$gPATTERN,$gPATTERN  #BEGIN," "$file"
   [ $? = 0 ] && rm "${file}.bak" || {

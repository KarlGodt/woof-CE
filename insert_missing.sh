...
do
   ...
   *.txt|*.TXT|*rc|*.xpm|*.svg|*.png|*.jpg|*.wav|*.au|*.bac|*.dat|*.pupdev|*.gs|*Help)
   ...
   if ! grep $QUIET '_VERSION_=' "$file"; then
   grep $QUIET -m1 '_TITLE_=' "$file" || continue

   lineNR1=`grep -n -m1 '_TITLE_=' "$file" | awk -F'[-:+]' '{print $1}'`
   [ "$lineNR1" ] || continue
   [ "${lineNR1//[0-9]/}" ] && continue

    if [ ! "$DRY" ]; then
    sPATTERN1="$lineNR1 a "'\\'"_VERSION_=1.0omega"
   ...
   if ! grep $QUIET 'MY_PID=' "$file"; then
   grep $QUIET -m1 'MY_SELF=' "$file" || continue

   lineNR2=`grep -n -m1 'MY_SELF=' "$file" | awk -F'[-:+]' '{print $1}'`
   [ "$lineNR2" ] || continue
   [ "${lineNR2//[0-9]/}" ] && continue

    if [ ! "$DRY" ]; then
    sPATTERN2="$lineNR2 a "'\\'"MY_PID=\$\$"
    sed -i".bak" "$sPATTERN2" "$file"
    [ $? = 0 ] && rm "${file}.bak" || {
    defaulttexteditor "$file"
    break
    }

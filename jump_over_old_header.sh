...
do
   [ -L "$file" ] && continue
   [ -f "$file" ] || continue
   file "$file" | grep -i text | grep $QUIET -viE 'ISO-8859|murga\-lua\-dynamic|perl|python|pascal|bacon|ASCII C program|C++ program|HTML document|X pixmap image|PNG image data' || { [ "$QUIET" ] || echo "$file is not a shell script file by detection"; continue; }
   case "$file" in
   *.txt|*.TXT|*rc|*.xpm|*.svg|*.png|*.jpg|*.wav|*.au|*.bac|*.dat|*.pupdev|*.gs|*Help)
   [ "$QUIET" ] || echo "$file is not a shell script file by EXT"; continue;;
   esac

   gPATTERN1='__old_header__(){'
   grep $QUIET "$gPATTERN1" "$file" && { [ "$QUIET" ] || echo "$file has '$gPATTERN1'"; continue; }
   gPATTERN2='f4puppy5'
   grep $QUIET "$gPATTERN2" "$file" || { [ "$QUIET" ] || echo "$file has no '$gPATTERN2'"; continue; }
   ...
   lineNR1=`grep $QUIET -n -m1 "$gPATTERN90" "$file" | tail -n1 | awk -F'[: ]' '{print $1}'`
   echo -e "\\033[1;31mlineNR1=$lineNR1'\\033[0;39m"
   [ "$lineNR1" ] || continue
   ...
   lineNR2=`grep $QUIET -n "$gPATTERN91" "$file" | tail -n1 | awk -F'[: ]' '{print $1}'`
   echo -e "\\033[1;33mlineNR2=$lineNR2'\\033[0;39m"
   [ "$lineNR2" ] || continue

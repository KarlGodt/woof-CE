...
showProfilesWindow()
{
    ...
    ifconfig -a | grep $Q -w "^$INTERFACE" || return 125
    local RV
    ...
    do
        ...
        case $STATEMENT in ''|*abort*|*Cancel*) return 123;; esac
    ...
    return ${RV:-1} # configureWireless() in net-setup.sh : 0 does testInterface, 2 Error message, * msg Canceled
...
useProfile ()
{
    ...
    case "$CURRENT_CONTEXT" in wag-profiles.sh) setupDHCP; RV=$?; esac
return ${RV:-0}
} # end useProfile
...
killWpaSupplicant ()
{
    ...
    [ "$INTERFACE" ] || INTERFACE="$1"
    #SUPPLICANT_PIDS=$( /bin/ps -e | grep -v "grep" | grep -E "wpa_supplicant.+${INTERFACE}" | grep -oE "^ *[0-9]+")

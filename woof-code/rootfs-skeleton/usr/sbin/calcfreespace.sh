...
case $PUPMODE in
 3|7|13) #home partition/file mntd on pup_ro1, tmpfs on pup_rw
  SIZEFREE=`/bin/df -k | grep ' /initrd/pup_ro1$' | tr -s ' ' | cut -f 4 -d ' '`
  SIZETMP=`/bin/df -k | grep ' /initrd/pup_rw$' | tr -s ' ' | cut -f 4 -d ' '`
 ...
 6|12) #home partition/file mntd on pup_rw (no tmpfs)
  SIZEFREE=`/bin/df -k | grep ' /initrd/pup_rw$' | tr -s ' ' | cut -f 4 -d ' '`
  ;;
 *)
  SIZEFREE=`/bin/df -k | grep ' /$' | tr -s ' ' | cut -f 4 -d ' '`
  ;;
esac

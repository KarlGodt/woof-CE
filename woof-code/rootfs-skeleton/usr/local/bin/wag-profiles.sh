...
setupDHCP()
{
        ...
        mkdir $VERB -p /var/lib/dhcpcd && touch /var/lib/dhcpcd
   ...
   <button>
     ...
     <action>EXIT:Abort</action>
        ...
        (
                ...
                if dhcpcd $DHCPCD_COMMON_OPS -t $MAX_TIME -d -I '' "$INTERFACE" 2>&1 | tee "$tmpDIR"/dhcpcd.log
                then
                        HAS_ERROR=0
                        grep $Q 'Error' "$tmpDIR"/dhcpcd.log && HAS_ERROR=1
                else
                        HAS_ERROR=1
                fi
...
killDhcpcd(){
        ...
        if [ -d /var/lib/dhcpcd ] ; then
          if [ -s /var/run/dhcpcd-${INTERFACE}.pid ] ; then
            ...
            rm $VERB -f /var/run/dhcpcd-${INTERFACE}.* 2>/dev/null
          fi
#begin rerwin - Retain duid, if any, so all interfaces can use
          ...
          rm $VERB -f /var/lib/dhcpcd/dhcpcd-${INTERFACE}.* 2>/dev/null  #.info
#end rerwin
        ...
        if [ -d /etc/dhcpc ];then
          if [ -s /etc/dhcpc/dhcpcd-${INTERFACE}.pid ] ; then
            ...
            rm $VERB /etc/dhcpc/dhcpcd-${INTERFACE}.pid 2>/dev/null
          fi
          rm $VERB /etc/dhcpc/dhcpcd-${INTERFACE}.* 2>/dev/null
...
buildScanWindow()
{
        ...
        (
                ...
                rm $VERB "$tmpDIR"/net-setup_scan*.tmp >/dev/null 2>&1
...
runPrismScan()
{
        ...
        wlanctl-ng "$INTERFACE" dot11req_scan bsstype=any \
         bssid=ff:ff:ff:ff:ff:ff ssid="" scantype=both \
          channellist="00:01:02:03:04:05:06:07:08:09:0a:0b:00:00" \
           minchanneltime=200 maxchanneltime=250 >"$tmpDIR"/prism-scan-all 2>/dev/null
        echo "X"
        ...
        rm $VERB "$tmpDIR"/prism-scan-all >/dev/null 2>&1
        ...
        if [ "$POINTNUM" ] ; then
          ...
          do
            wlanctl-ng "$INTERFACE" dot11req_scan_results bssindex=$P >"$tmpDIR"/prism-scan$P 2>/dev/null
          done
          echo "X"
...
buildPrismScanWindow()
{
  ...
  (
        ...
        if runPrismScan "$INTERFACE" ; then
          ...
          for P in $(seq 0 $POINTNUM)
          do
            grep -F $Q 'resultcode=success' "$tmpDIR"/prism-scan$P || continue

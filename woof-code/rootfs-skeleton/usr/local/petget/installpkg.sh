...
DIRECTSAVEPATH=""
#111013 shinobar: this currently not working, bypass for now...
...
do
 ...
 if [ "$ICON" != "" ];then
  ...
  FNDICON="`find /usr/local/lib/X11/mini-icons /usr/share/pixmaps -maxdepth 1 -name $ICONBASE -o -name $ICONBASE.png -o -name $ICONBASE.xpm -o -name $ICONBASE.jpg | head -n 1`"
  if [ "$FNDICON" ];then
   ...
   #look elsewhere...
   FNDICON="`find /usr/share/icons /usr/local/share/pixmaps /usr/share/pixmaps -name $ICONBASE -o -name $ICONBASE.png -o -name $ICONBASE.xpm -o -name $ICONBASE.jpg | head -n 1`"
   if [ "$FNDICON" ];then
    ICONNAMEONLY="`basename "$FNDICON"`"

#!/bin/sh
#
# New header by Karl Reimer Godt, September 2014
  _TITLE_="Puppy_dependencies.sh"
_VERSION_=1.0omega
_COMMENT_="$_TITLE_:Puppy Linux shell script [to TODO here]"

MY_SELF="/usr/local/petget/dependencies.sh"
MY_PID=$$

test -f /etc/rc.d/f4puppy5 && {
[ "$HAVE_F4PUPPY5" ] || source /etc/rc.d/f4puppy5

ADD_PARAMETER_LIST=""
ADD_PARAMETERS=""
_provide_basic_parameters

TWO_HELP=''; TWO_VERSION=''; TWO_VERBOSE=''; TWO_DEBUG=''; ## Set to anything if code requires further down (ie. custom usage or version message)
ADD_HELP_MSG="$_COMMENT_"
_parse_basic_parameters "$@"
[ "$DO_SHIFT" ] && [ ! "${DO_SHIFT//[[:digit:]]/}" ] && {
  for oneSHIFT in `seq 1 1 $DO_SHIFT`; do shift; done; }

_trap

}
# End new header
#
#called from /usr/local/petget/installpreview.sh
#/tmp/PetGet/petget_installpreview_pkgname (writen in installpreview.sh) has name of package being
#  previewed prior to installation. ex: abiword-1.2.3
#/tmp/PetGet/petget_filterversion has the repository that installing from (written in pkgchooser.sh).
#  ex: slackware-12.2-slacky
#  ...full package database file is /root/.packages/Packages-slackware-12.2-slacky
#/tmp/PetGet/petget_missingpkgs_patterns (written in findmissingpkgs.sh) has a list of missing dependencies, format ex:
#  |kdebase|
#  |kdelibs|
#  |mesa-lib|
#  |qt|
#  ...that is, pkg name-only with vertical-bars on both ends, one name per line.
#/tmp/PetGet/petget_installed_patterns_all (writen in findmissingpkgs.sh) has a list of already installed
#  packages, both builtin and user-installed. One on each line, exs:
#  |915resolution|
#  |a52dec|
#  |absvolume_puppy|
#  |alsa\-lib|
#  |cyrus\-sasl|
#  ...notice the '-' are backslashed.

. /root/.packages/PKGS_MANAGEMENT #has DISTRO_PPM_DEVX_EXCEPTIONS, PKG_ALIASES_INSTALLED


#a problem is that the dependencies may have their own dependencies. Some pkg
#databases have all dependencies up-front, whereas some only list the higher-level
#dependencies and the dependencies of those have to be looked for.

yaf-splash -font "8x16" -outline 0 -margin 4 -bg orange -text "Please wait, processing package database files..." &
X1PID=$!

ALLINSTALLEDPKGS="`cat /tmp/PetGet/petget_installed_patterns_all`"
TREE1="`cat /tmp/PetGet/petget_installpreview_pkgname`"

#this is the db of the main pkg...
DB_MAIN="/root/.packages/Packages-`cat /tmp/PetGet/petget_filterversion`" #ex: Packages-slackware-12.2-official
#...should have first preference when looking for dependencies...
DB_OTHERS="`ls -1 /root/.packages/Packages-* | grep -v "$DB_MAIN"`"
#if DB_MAIN is puppy-4, puppy-3 or puppy-2, then only look in those...
if [ "`echo "$DB_MAIN" | grep '\\-puppy\\-2'`" != "" ];then
 DB_OTHERS="`echo "$DB_OTHERS" | grep '\\-puppy\\-2'`"
else
 if [ "`echo "$DB_MAIN" | grep '\\-puppy\\-3'`" != "" ];then
  DB_OTHERS="`echo "$DB_OTHERS" | grep '\\-puppy\\-3'`"
 else
  if [ "`echo "$DB_MAIN" | grep '\\-puppy\\-4'`" != "" ];then
   DB_OTHERS="`echo "$DB_OTHERS" | grep '\\-puppy\\-4'`"
  else
   #do not look in puppy-2, puppy-3 or puppy-4...
   DB_OTHERS="`echo "$DB_OTHERS" | grep -v '\\-puppy\\-[234]'`"
  fi
 fi
fi
DB_OTHERS="`echo "$DB_OTHERS" | tr '\n' ' '`"

#the question is, how deep to search for deps? i'll go down 2 levels... make it 3...
cp -f /tmp/PetGet/petget_missingpkgs_patterns /tmp/PetGet/petget_missingpkgs_patternsx
echo "HIERARCHY OF MISSING DEPENDENCIES OF PACKAGE $TREE1" > /tmp/PetGet/petget_deps_visualtreelog #w017
echo "Format of each line: 'a-missing-dependent-pkg: missing dependencies of a-missing-dependent-pkg'" >> /tmp/PetGet/petget_deps_visualtreelog #w017
for ONELEVEL in 1 2 3
do
 echo "" >> /tmp/PetGet/petget_deps_visualtreelog #w017
 echo -n "" > /tmp/PetGet/petget_missingpkgs_patterns2
 for depPATTERN in `cat /tmp/PetGet/petget_missingpkgs_patternsx`
 do
  ONEDEP="`echo -n "$depPATTERN" | sed -e 's%|%%g'`" #convert to exact name, ex: abiword
  depPATTERN="`echo -n "$depPATTERN" | sed -e 's%\\-%\\\\-%g'`" #backslash '-'
  #find database entry for this package...
  for ONEDB in $DB_MAIN $DB_OTHERS
  do
   DB_dependencies="`cat $ONEDB | cut -f 1,2,9 -d '|' | grep "$depPATTERN" | cut -f 3 -d '|' | head -n 1 | sed -e 's%,$%%'`"
   if [ "$DB_dependencies" != "" ];then
    ALLDEPS_PATTERNS="`echo -n "$DB_dependencies" | tr ',' '\n' | grep '^+' | sed -e 's%^+%%' -e 's%$%|%' -e 's%^%|%'`" #put '|' on each end.
    echo "$ALLDEPS_PATTERNS" > /tmp/PetGet/petget_subpkg_deps_patterns
    MISSINGDEPS_PATTERNS="`grep --file=/tmp/PetGet/petget_installed_patterns_all -v /tmp/PetGet/petget_subpkg_deps_patterns`"
    echo "$MISSINGDEPS_PATTERNS" >> /tmp/PetGet/petget_missingpkgs_patterns2
    #w017 log a visual tree...
    MISSDEPSLIST="`echo "$MISSINGDEPS_PATTERNS" | sed -e 's%|%%g' | tr '\n' ' '`"
    case $ONELEVEL in
     1)
      echo "$ONEDEP: $MISSDEPSLIST" >> /tmp/PetGet/petget_deps_visualtreelog
     ;;
     2)
      echo "    $ONEDEP: $MISSDEPSLIST" >> /tmp/PetGet/petget_deps_visualtreelog
     ;;
     3)
      echo "        $ONEDEP: $MISSDEPSLIST" >> /tmp/PetGet/petget_deps_visualtreelog
     ;;
    esac
    break
   fi
  done
 done
 sort -u /tmp/PetGet/petget_missingpkgs_patterns2 > /tmp/PetGet/petget_missingpkgs_patternsx
 cat /tmp/PetGet/petget_missingpkgs_patternsx >> /tmp/PetGet/petget_missingpkgs_patterns #accumulate them.
done
sort -u /tmp/PetGet/petget_missingpkgs_patterns > /tmp/PetGet/petget_missingpkgs_patternsx
mv -f /tmp/PetGet/petget_missingpkgs_patternsx /tmp/PetGet/petget_missingpkgs_patterns

#now find the entries in the databases...
rm -f /tmp/PetGet/petget_missing_dbentries* 2>/dev/null
for depPATTERN in `cat /tmp/PetGet/petget_missingpkgs_patterns`
do
 depPATTERN="`echo -n "$depPATTERN" | sed -e 's%\\-%\\\\-%g'`" #backslash '-'.
 for ONEREPODB in $DB_MAIN $DB_OTHERS
 do
  DBFILE="`basename $ONEREPODB`" #ex: Packages-slackware-12.2-official
  #find database entry(s) for this package...
  DB_ENTRY="`cat $ONEREPODB | grep "$depPATTERN"`"
  if [ "$DB_ENTRY" != "" ];then
   echo "$DB_ENTRY" >> /tmp/PetGet/petget_missing_dbentries-${DBFILE}-2
   break
  fi
 done
done
#clean them up...
for ONEREPODB in $DB_MAIN $DB_OTHERS
do
 DBFILE="`basename $ONEREPODB`" #ex: Packages-slackware-12.2-official
 if [ -f /tmp/PetGet/petget_missing_dbentries-${DBFILE}-2 ];then
  sort -u /tmp/PetGet/petget_missing_dbentries-${DBFILE}-2 > /tmp/PetGet/petget_missing_dbentries-${DBFILE}
  rm -f /tmp/PetGet/petget_missing_dbentries-${DBFILE}-2
 fi
done

kill $X1PID

###END###


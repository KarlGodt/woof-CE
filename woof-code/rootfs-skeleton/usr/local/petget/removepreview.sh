...
grep -v "$remPATTERN" /root/.packages/user-installed-packages > /tmp/PetGet/petget-user-installed-pkgs-rem
cp -f /tmp/PetGet/petget-user-installed-pkgs-rem /root/.packages/user-installed-packages
...
echo -n "" > /tmp/PetGet/petget-deps-maybe-rem
...
do
 ...
 [ "`echo "$DEP_PKGS" | grep "$opPATTERN"`" != "" ] && echo "$ONE_pkgname DESCRIPTION: $ONE_description" >> /tmp/PetGet/petget-deps-maybe-rem
done
EXTRAMSG=""
if [ -s /tmp/PetGet/petget-deps-maybe-rem ];then
 #MAYBEREM="`cat /tmp/PetGet/petget-deps-maybe-rem`" # wrap=\"false\"
 #nah, just list the names, not descriptions...
 MAYBEREM="`cat /tmp/PetGet/petget-deps-maybe-rem | cut -f 1 -d ' ' | tr '\n' ' '`"

...
"
 ...
 echo 'There are no user-installed packages yet, so nothing to uninstall!'
 fi
 exit
fi
...
if [ "$DISPLAY" != "" ];then
 ...
 echo "UNINSTALL '$DB_pkgname' ?"
 read -t 60 yesno
 yesno=`echo "$yesno" | tr '[[:upper:]]' '[[:lower:]]'`
 case $yesno in
 y|yes) echo "OK , uninstalling ...";;
 *)exit;;
 esac
fi
 ...
 do
  if [ ! -d "$ONESPEC" ];then
   if [ -e "/initrd/pup_ro2$ONESPEC" ];then
    ...
    rm -f "$ONESPEC"
    [ -f "ONESPEC".backup.${DB_pkgname} ] && mv "ONESPEC".backup.${DB_pkgname} "$ONESPEC"
   fi
  fi
 done

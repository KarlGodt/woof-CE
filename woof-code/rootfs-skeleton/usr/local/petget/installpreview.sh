...
#100821 bug in Lucid 5.1, /tmp/PetGet/pup_event_sizefreem had two identical lines.
...
rm -f /tmp/PetGet/petget_missing_dbentries-* 2>/dev/null
...
SIZEFREEM=`cat /tmp/PetGet/pup_event_sizefreem | head -n 1` #100821 bug in Lucid 5.1, file had two identical lines.
...
if [ "$DB_dependencies" = "" ];then
 ...
 MISSINGDEPS_PATTERNS="`cat /tmp/PetGet/petget_missingpkgs_patterns`"
 ...
 if [ "$MISSINGDEPS_PATTERNS" = "" ];then
  ...
  DEPBUTTON="<button>
   ...
   <action>echo \"${TREE1}\" > /tmp/PetGet/petget_installpreview_pkgname</action>
...
if [ "$EXIT" = "BUTTON_EXAMINE_DEPS" ];then
 ...
 #returns with /tmp/PetGet/petget_missing_dbentries-* has the database entries of missing deps.
 ...
 echo -n "" > /tmp/PetGet/petget_moreframes
 echo -n "" > /tmp/PetGet/petget_tabs
 echo "0" > /tmp/PetGet/petget_frame_cnt
 DEP_CNT=0
 ONEREPO=""
 for ONEDEPSLIST in `ls -1 /tmp/PetGet/petget_missing_dbentries-*`
 do
  ONEREPO_PREV="$ONEREPO"
  ONEREPO="`echo "$ONEDEPSLIST" | grep -o 'Packages.*' | sed -e 's%Packages\\-%%'`"
  FRAME_CNT=`cat /tmp/PetGet/petget_frame_cnt`
  if [ "$ONEREPO_PREV" != "" ];then #next repo, so start a new tab.
   DEP_CNT=0
   FRAME_CNT=`expr $FRAME_CNT + 1`
   echo "$FRAME_CNT" > /tmp/PetGet/petget_frame_cnt
   #w017 bugfix, prevent double frame closure...
   [ "`cat /tmp/PetGet/petget_moreframes | tail -n 1 | grep '</frame>$'`" = "" ] && echo "</frame>" >> /tmp/PetGet/petget_moreframes
  fi
  cat $ONEDEPSLIST |
  while read ONELIST
  do
   ...
   case $DEP_CNT in
    1)
     echo -n "<frame REPOSITORY: ${ONEREPO}>" >> /tmp/PetGet/petget_moreframes
     echo -n "Dependencies|" >> /tmp/PetGet/petget_tabs
     echo -n "<checkbox><default>true</default><label>${DEP_NAME} SIZE: ${DEP_SIZE}B DESCRIPTION: ${DEP_DESCR}</label><variable>CHECK_PKG_${ONEREPO}_${DEP_NAME}</variable></checkbox>" >> /tmp/PetGet/petget_moreframes
    ;;
    8)
     FRAME_CNT=`cat /tmp/PetGet/petget_frame_cnt`
     FRAME_CNT=`expr $FRAME_CNT + 1`
     if [ $FRAME_CNT -gt 5 ];then
      echo -n "<text use-markup=\"true\"><label>\"<b>SORRY! Too many dependencies, list truncated. Suggest install some deps first.</b>\"</label></text>" >> /tmp/PetGet/petget_moreframes
     else
      echo -n "<checkbox><default>true</default><label>${DEP_NAME} SIZE: ${DEP_SIZE}B DESCRIPTION: ${DEP_DESCR}</label><variable>CHECK_PKG_${ONEREPO}_${DEP_NAME}</variable></checkbox>" >> /tmp/PetGet/petget_moreframes
     fi
     echo "</frame>" >> /tmp/PetGet/petget_moreframes
     DEP_CNT=0
     echo "$FRAME_CNT" > /tmp/PetGet/petget_frame_cnt
    ;;
    *)
   ...
   echo "$INSTALLEDSIZEK" > /tmp/PetGet/petget_installedsizek
  done
  INSTALLEDSIZEK=`cat /tmp/PetGet/petget_installedsizek`
  FRAME_CNT=`cat /tmp/PetGet/petget_frame_cnt`
  [ $FRAME_CNT -gt 5 ] && break #too wide!
 done
 TABS="`cat /tmp/PetGet/petget_tabs`"
 MOREFRAMES="`cat /tmp/PetGet/petget_moreframes`"
  ...
  <vbox>
   ...
   <hbox>
    <button>
     ...
     <action>/usr/local/bin/defaulttextviewer /tmp/PetGet/petget_deps_visualtreelog & </action>
    </button>
   </hbox>
  </vbox>
...
"
 ...
 if [ "`echo "$RETPARAMS" | grep '^EXIT' | grep -E 'BUTTON_PKGS_INSTALL|BUTTON_PKGS_DOWNLOADONLY'`" != "" ];then
  ...
  do
   ...
   opPATTERN='^'"$ONEPKG"'|'
   grep -v "$opPATTERN" /tmp/PetGet/petget_missing_dbentries-Packages-${ONEREPO} > /tmp/PetGet/petget_tmp
   mv -f /tmp/PetGet/petget_tmp /tmp/PetGet/petget_missing_dbentries-Packages-${ONEREPO}
  done
 else
  exit
 fi
...
touch /tmp/PetGet/petget_missing_dbentries-${DB_FILE} #create if doesn't exist.
mPATTERN='^'"$TREE1"'|'
if [ "`grep "$mPATTERN" /tmp/PetGet/petget_missing_dbentries-${DB_FILE}`" = "" ];then
 echo "$DB_ENTRY" >> /tmp/PetGet/petget_missing_dbentries-${DB_FILE}
fi
...
[ "`cat /tmp/PetGet/petget-installed-pkgs-log | grep -o 'CATEGORY' | grep -v 'none'`" = "" ] && INSTALLEDCAT="none"
...
iUPDATE='no'
for iONE in `cat /tmp/PetGet/petget_missing_dbentries-* | cut -f 1 -d '|' | tr '\n' ' '`
do
...
PKGS="`cat /tmp/PetGet/petget_missing_dbentries-* | cut -f 1 -d '|' | tr '\n' '|'`"

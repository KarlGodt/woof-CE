...
cp -f "$tmpDIR"/filterpkgs.results "$tmpDIR"/filterpkgs.results.post

#120525 quick filtering but not perfect...
#PETs: _DEV _DOC _NLS
#Ubuntu,Debian DEBs: -dev_ -doc_ -docs_ -langpack -lang-
#Mageia RPMs: -devel- -doc-
sed -i -e '/-dbg_/d' "$tmpDIR"/filterpkgs.results.post #120525 always take out the debug pkgs.
...
if [ "$DEF_CHK_EXE" = "false" ];then
 ...
 mv -f "$tmpDIR"/filterpkgs.results.post.tmp "$tmpDIR"/filterpkgs.results.post
fi

##120529 append an icon to each entry...
#cp -f "$tmpDIR"/filterpkgs.results.post "$tmpDIR"/filterpkgs.results.post-noicons
...
# sed -i -r -e 's%(^[a-zA-Z]*)%\1|\1%' "$tmpDIR"/filterpkgs.results.post
#fi
...
#sed -i -e 's%|%|mini-%' "$tmpDIR"/filterpkgs.results.post
###120813 finds first ; followed by a character, appends "mini-" before the char...
##sed -i -e 's%|%|mini-%' -r -e 's%;([a-z])%;mini-\1%' "$tmpDIR"/filterpkgs.results.post

#120817 category field: Document;edit becomes mini-Document-edit...
sed -i -e 's%|%|mini-%' -e 's%;%-%' "$tmpDIR"/filterpkgs.results.post

...
#You detonate
 ...
 while :; do
   ...
   *'You successfully disarm '*)
    ...
    ## but needs to read if more traps were there and either successfully disarmed or failed
    ;;

   *'You fail to disarm '*)
    #  break ## break 1 tries again to disarm
    ## but needs to fall through to check if set off
    ;;

   *'In fact, you set it off!'*)
    #  break ## break 1 tries again to disarm
    ## but before disarming again, needs to read yeld of trap
    ;;

    #You set off a fireball!
   *off*fireball*)  ## rune_fireball.arc
...
read -t 1 SECONDLINE
_debug "$SECONDLINE"
_log "_handle_trap_easy:$SECONDLINE"
if [ "$FORCE" ]; then
  break # always better to exit with beep
...
read -t 1 SECONDLINE
_debug "$SECONDLINE"
_log "_handle_trap_medium:$SECONDLINE"
if [ "$FORCE" \> 1 ]; then
  break # always better to exit with beep
else _draw 3 "Quitting - $MSG."
...
read -t 1 SECONDLINE
_debug "$SECONDLINE"
_log "_handle_trap_destroy:$SECONDLINE"
if [ "$FORCE" \> 2 ]; then
  break # always better to exit with beep
else _draw 3 "Quitting - $MSG."

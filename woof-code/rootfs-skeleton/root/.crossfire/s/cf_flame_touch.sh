...
VERSION=3.4 # Wrap the call for _main_flame_touch* into a while loop
...
_draw_stdalone 7  "by skill flame touch ( fireborn )."
...
_draw 7  "by skill flame touch ( fireborn )."
...
# d   :debugging output
while getopts A:BC:D:LZdVhabdefgijklmnopqrstuvwxyzEFGHIJKLMNOPQRSTUWXY oneOPT
do
case $oneOPT in
...
# brace and
# attack around
# with skill flame touch

_flame_touch_around_stdalone(){
_msg_stdalone 7 "_flame_touch_around_stdalone:$*"
_log_stdalone   "_flame_touch_around_stdalone:$*"
...
_check_skill_available_stdalone "flame touch" || return 1
fi
...
_main_flame_touch_stdalone(){
_msg_stdalone 7 "_main_flame_touch_stdalone:$*"
_log_stdalone   "_main_flame_touch_stdalone:$*"
...
_debug "_main_flame_touch_func:$*"
_log   "_main_flame_touch_func:$*"
...
if _check_skill_available "flame touch"; then
[ "$DO_BRACE" ] && _brace
_flame_touch_around
_unbrace
fi

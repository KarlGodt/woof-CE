...
# 2018-02-10
# cf_karate.sh :
# script to level up the skill karate
...
VERSION=3.4 # Wrap the call for _main_karate* into a while loop
...
_draw_stdalone 7  "by skill karate."
...
# d   :debugging output
while getopts A:BC:D:LZdVhabdefgijklmnopqrstuvwxyzABEFGHIJKMNOPQRSTUWXYZ oneOPT
do
case $oneOPT in
A) ATTACKS_SPOT=$OPTARG;;
B) DO_BRACE=1;;
...
_msg_stdalone 7 "_karate_around_stdalone:$*"
...
_check_skill_available_stdalone karate || return 1
fi
...
_msg_stdalone 7 "_main_karate_stdalone:$*"
...
if _check_skill_available_stdalone karate; then
...
if _check_skill_available karate; then
[ "$DO_BRACE" ] && _brace
_karate_around
_unbrace
fi

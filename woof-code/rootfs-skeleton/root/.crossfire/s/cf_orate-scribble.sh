...
_draw 8  "Options:"
_draw 10 "Simple number as first parameter:Just make NUMBER followers."
_draw 10 "-C # :like above, just make NUMBER followers."
...
# S # :Singing attempts
# d   :debugging output
while getopts C:O:S:dVhabdefgijklmnopqrstuvwxyzABDEFGHIJKLMNPQRTUWXYZ oneOPT
do
case $oneOPT in
C) NUMBER=$OPTARG;;
...
_debug "$REPLY"
 ...
 case $REPLY in
 '') break 1;;
 #*'You ready the skill '"$lSKILL"*) lRV=0; break 1;;  # "You ready the spell %s", spob->name
...
_draw 2 "Turned to direction $DIRECTION .."
}
...
__sing_and_orate_around_stdalone(){
_debug "__sing_and_orate_around_stdalone:$*"
...
_set_next_direction_stdalone
 ...
 do
  ...
  _is 1 1 fire $DIRN
  _is 1 1 fire_stop
  SINGING_ATTEMPTS=$((SINGING_ATTEMPTS+1))
  _sleep

  while :; do unset REPLY
  read -t $TMOUT
  _log "ready_skill singing:$REPLY"
  _debug "$REPLY"

  case $REPLY in
  #'') _kill_monster_stdalone; break 1;;
  '') break 2;;
  ...
  *'You sing'*)
   ...
   _log "ready_skill singing:$REPLY"
   _debug "$REPLY"
    case $REPLY in *'You calm down the '*)
    CALMS=$((CALMS+1)); break 2;;
    #'') _set_next_direction_stdalone; break 1;;
    '') break 2;;
    *scripttell*break*)     break ${REPLY##*?break};;
    *scripttell*exit*)    _exit 1;;
    esac
  ...
  *'You calm down the '*) CALMS=$((CALMS+1)); break 2;;
...
_draw 2 "You calmed ${CALMS:-0} monsters."
_sleep
test "$one" -ge $((CALMS+9)) && break 1
 ...
 do
  ...
  _is 1 1 fire $DIRN
  _is 1 1 fire_stop
  ORATORY_ATTEMPTS=$((ORATORY_ATTEMPTS+1))
  _sleep

  while :; do unset REPLY
  read -t $TMOUT
  _log "ready_skill oratory:$REPLY"
  _debug "$REPLY"

  case $REPLY in
  '') break 2;; # monster does not respond at all, try next
  ...
  *'You orate to the '*)
    ...
    case $REPLY in
    *'Your follower loves '*)  break 2;;
    *'Too bad '*)              break 2;;
    *'You convince the '*) FOLLOWS=$((FOLLOWS+1)); break 2;;
    *scripttell*break*)     break ${REPLY##*?break};;
    *scripttell*exit*)    _exit 1;;
    esac
    break 1;; # monster does not respond, try again

  *'Your follower loves '*)  break 2;; # next creature or exit
  *'Too bad '*)              break 2;; # try again singing the kobold isn't listening!
  *'You convince the '*' to become your follower.'*) FOLLOWS=$((FOLLOWS+1)); break 2;; # next monster
  *'There is nothing to orate to.'*) break 2;;
  *scripttell*break*)     break ${REPLY##*?break};;
  *scripttell*exit*)    _exit 1;;
  *) :;;
  esac
...
_got_sing_reply_ash_stdalone(){
_debug "_got_sing_reply_ash_stdalone:$*"

   unset REPLY
   read -t $TMOUT
   _log "_got_sing_reply_ash_stdalone:$REPLY"
   _debug "$REPLY"

    case $REPLY in
    *'You calm down the '*) CALMS=$((CALMS+1)); break 2;;
    *"Too bad the "*"isn't listening!"*) break 2;;  # impossible to calm down (again)
    #'') _set_next_direction_stdalone; break 1;;
    '') break 2;;
    *scripttell*break*)     break ${REPLY##*?break};;
    *scripttell*exit*)    _exit 1;;
...
_got_sing_reply_stdalone(){
_debug "_got_sing_reply_stdalone:$*"

local lRV=

   while :; do unset REPLY
   read -t $TMOUT
   _log "_got_sing_reply_stdalone:$REPLY"
   _debug "$REPLY"

    case $REPLY in
    *'You calm down the '*) CALMS=$((CALMS+1)); lRV=0;;
    *"Too bad the "*"isn't listening!"*) lRV=4;; # impossible to calm down
    #'') _set_next_direction_stdalone; break 1;;
    '') lRV=${lRV:-1}; break 1;;
    *scripttell*break*)     break ${REPLY##*?break};;
    *scripttell*exit*)    _exit 1;;
...
_watch $DRAWINFO
 ...
 do
  ...
  #    case $? in
  #    0)  break 2;; # success or not there; next monster
  #    1)  _kill_monster_stdalone; break 2;; # no success, kill; next monster
  #    *)  break 2;; # empty response, noise, other; next monster
  #    esac
   '') break 2;; #!=PLAYER
  #;;
  #*'You sing'*) _got_sing_reply_stdalone
  #              case $? in
  #    0) lRV=0; break 2;;
  #    1) TRY_OR=$((TRY_OR+1))
  #       test "$TRY_OR" -ge ${TRY_ORATE_MAX:-5} && { _kill_monster_stdalone; break 2; }
  #
  #       _orate_to_monster_ready_skill_stdalone
  #       case $? in
  #       0)  break 2;; # success or not there; next monster
  ...
  #  2) :;; # syntax error
  #  3) :;; # other REPLY
  #  4) _kill_monster_stdalone; break 2;;
  #  *) :;;
  #  esac
  *'You sing'*) break 1;;
  #;;
...
_unwatch $DRAWINFO
_draw 5 "With ${SINGING_ATTEMPTS:-0} singings you calmed down ${CALMS:-0} monsters."
_sleep
_draw 3 "lRV=$lRV"
return ${lRV:-1}
}

_got_orate_reply_ash_stdalone(){
_debug "_got_orate_reply_ash_stdalone:$*"

    unset REPLY
    read -t $TMOUT
    _log "_got_orate_reply_ash_stdalone:$REPLY"
    _debug "$REPLY"

    case $REPLY in
    *'Your follower loves '*)  break 2;; #FLAG_FRIENDLY, PETMOVE
    *'Too bad '*) _debug "Caught Too bad 2"; break 2;; #!FLAG_UNAGGRESSIVE, !FLAG_FRIENDLY
    *'You convince the '*) FOLLOWS=$((FOLLOWS+1)); break 2;;
    *'There is nothing to orate to.'*) break 2;; #FLAG_MONSTER
    '') :;;
    *scripttell*break*)     break ${REPLY##*?break};;
    *scripttell*exit*)    _exit 1;;
    *) :;;
...
_got_orate_reply_stdalone(){
_debug "_got_orate_reply_stdalone:$*"

local lRV=

    unset REPLY
    read -t $TMOUT
    _log "_got_orate_reply_stdalone:$REPLY"
    _debug "$REPLY"

    case $REPLY in
    *'Your follower loves '*)  lRV=0;;
    *'There is nothing to orate to.'*) lRV=0;;
    *'You convince the '*) FOLLOWS=$((FOLLOWS+1)); lRV=0;; #SET_FLAG(tmp,FLAG_FRIENDLY), PETMOVE

    "Your speech angers "*) _debug "Caught Anger 2";   lRV=1;; #CLEAR_FLAG(tmp,FLAG_FRIENDLY), CLEAR_FLAG(tmp,FLAG_UNAGGRESSIVE)
    *'Too bad '*)           _debug "Caught Too bad 2"; lRV=1;; #FLAG_UNAGGRESSIVE, FLAG_FRIENDLY
    '') :;;
    *scripttell*break*)     break ${REPLY##*?break};;
    *scripttell*exit*)    _exit 1;;
    *) :;;
...
_draw 5 "With ${ORATORY_ATTEMPTS:-0} oratings you conceived ${FOLLOWS:-0} followers."
return ${lRV:-1}
}
...
case $? in
 0) _orate_to_monster_ready_skill_stdalone
  case $? in
  0) :;;
  *) _kill_monster_stdalone;; # continue;;
  esac
 ;;
 *) :;;
esac

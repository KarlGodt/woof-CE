...
# FOREVER mode
INF_THRESH=10 # threshold to cast dexterity and probe
...
readonly DIR DIRN && test "$DIR" -a "$DIRN"
...
#  *'You fail to disarm '*) continue;;
   *'You successfully disarm '*)
    DISARM_SUCCESS=$((DISARM_SUCCESS+1))
...
_summary(){
test "$SEARCH_SUCCESS" && _draw 8 "You successfully found '$SEARCH_SUCCESS' trap(s)."
test "$DISARM_SUCCESS" && _draw 7 "You successfully disarmed '$DISARM_SUCCESS' trap(s)."
test "$LOCKPICK_ATT"   && _draw 5 "You did '$LOCKPICK_ATT' attempts to lockpick."
}

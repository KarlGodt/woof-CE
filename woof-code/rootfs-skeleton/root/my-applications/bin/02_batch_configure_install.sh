...
echo "Checking for include s in $i..." >>headers.check
header=''
includes=`grep -E -e '#include[[:blank:]]*.*|#[[:blank:]]*include[[:blank:]]*.*' $i |awk '{print $2 $3}' |sed 's%include%%;s%/\*.*%%' |tr -d '"' |tr -d '<' |tr -d '>' |awk '{print $1}'`
...
echo "$j local header"
echo "$j" >>local_headers.db
header=''
continue
else echo "Skipping $j for local header check.." >>headers.check
fi
else echo "NOT found local header $j..?" >>headers.check
fi
else
echo "$j already noted local header" >>headers.check
header=''
continue
fi #local header
...
if [ ! "$header" ];then
header=`find /usr/include -maxdepth $k -name "$j" |head -n1`
...
[ ! -s ./make.errs.log ] && rm -f ./make.errs.log
cd ..

...
cat -n /etc/rc.d/PUPSTATE  | tr '[[:blank:]]' ' ' | \
sed 's/^[[:blank:]]*// ; s/|/_with_/g' | \
sed "s/'\([[:alnum:]]\)/\1/g ; 
s/'\([[:blank:]]*[[:alnum:]]\)/\1/g ; 
s/'\([[:punct:]]*[[:alnum:]]\)/\1/g ; 
s/\([[:alnum:]]\)'/\1/g ; 
s/\([[:alnum:]][[:blank:]]*\)'/\1/g" > $Tmp_Dir/view_PUPSTATE_pre

##formatting the viewer input:(
#func(){
sed -i 's/\(^[[:digit:]]*\)\ /\1\|/g' $Tmp_Dir/view_PUPSTATE_pre
sed -i 's/\(^[[:digit:]]\)|/00\1|/g' $Tmp_Dir/view_PUPSTATE_pre
sed -i 's/\(^[[:digit:]][[:digit:]]\)|/0\1|/g' $Tmp_Dir/view_PUPSTATE_pre
sed -i 's/=/\|/g' $Tmp_Dir/view_PUPSTATE_pre
sed -i 's/\,/\|/g' $Tmp_Dir/view_PUPSTATE_pre
#}

cat -n $Tmp_Dir/view_PUPSTATE_pre | sed 's/^[[:blank:]]*//g' | \
grep -v -E -e '^[[:digit:]]*[[:blank:]]*[[:digit:]]*\|\#|^\#|^[[:blank:]]*\#' | \
tr '\t' ' ' > $Tmp_Dir/view_PUPSTATE_valid_lines
cat -n $Tmp_Dir/view_PUPSTATE_pre | sed 's/^[[:blank:]]*//g' | \
   grep -E -e '^[[:digit:]]*[[:blank:]]*[[:digit:]]*\|\#|^\#|^[[:blank:]]*\#' | \
tr '\t' ' ' > $Tmp_Dir/view_PUPSTATE_comment_lines
...
cd $Tmp_Dir
tar -czf "view_PUPSTATE.files.$TIME.tar.gz" view_PUPSTATE_pre view_PUPSTATE_valid_lines view_PUPSTATE_comment_lines view_PUPSTATE /etc/rc.d/PUPSTATE

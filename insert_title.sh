...
do
   ...
   *.txt|*.TXT|*rc|*.xpm|*.svg|*.png|*.jpg|*.wav|*.au|*.bac|*.dat|*.pupdev|*.gs|*Help)
   ...
   unset NEW_TITLE
   grep $QUIET -w 'dialog' "$file" && NEW_TITLE="puppy_${file##*/}"
   grep $QUIET 'gtkdialog' "$file" && NEW_TITLE="Puppy_${file##*/}"
   grep $QUIET 'Xdialog' "$file" && NEW_TITLE="Puppy_${file##*/}"

   [ "$NEW_TITLE" ] || NEW_TITLE="pup_${file##*/}.sh"

   if [ ! "$DRY" ]; then
   sed -i".bak" "s%_TITLE_=.*%_TITLE_=\"$NEW_TITLE\"%" "$file"
   [ $? = 0 ] && rm "${file}.bak" || {
